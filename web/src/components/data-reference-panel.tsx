...
const DATA_SECTIONS: Section[] = [
  ...
  },
  {
    title: "产品查询",
    items: [
      "查询 [输入ASIN] 产品详情：价格、评分、销量、BSR",
      "查询 [输入ASIN] 用户好评和差评",
      "查询 [输入ASIN] 产品销量趋势",
      "查询 [输入ASIN] 流量来源关键词",
      "查询 [输入ASIN] 产品变体：颜色、规格",
    ],
  },
  {
    title: "品类查询",
    items: [
      "查询 [输入关键词] 品类报告：Top 产品、品牌",
      "查询 [输入关键词] 品类趋势",
      "查询 [输入关键词] 品类关键词",
      "查询 [输入关键词] 品类子类目结构",
    ],
    ...
    >
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "0 14px 16px" }}>
        ...
        {DATA_SECTIONS.map((sec) => (
          <CollapsibleSection key={sec.title} title={sec.title} items={sec.items} onItemClick={onSelectCommand} />
        ))}
